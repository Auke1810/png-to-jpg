	...
	class png_to_jpg {
			...
			<table class="wp-list-table widefat striped media">
				...
				<tbody><?php
					foreach ( $query_images->posts as $image ) { ?>
						<tr data-id="<?php echo $image->ID ?>" data-url="<?php echo $image->guid ?>" data-transparency="-">
		...
		function convert_old_png(){
			if ( defined('DOING_AJAX') && DOING_AJAX ){
				...
				$image->path = $this->getFullPath( $image->guid );
				$params = array(
					"ID" => $image->ID,
					"file" => $image->path,
					"url" => $image->guid,
				...
				if( $this->convert_image( $params ) ){
					$this->delete_png_thumbnails( $image );
		...
		function delete_png_thumbnails( $image ){
